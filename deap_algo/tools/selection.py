...
def selBest(individuals, k, fit_attr="fitness"):
    ...
    """
    #print ("testing")
    #print (sorted(individuals, key=attrgetter(fit_attr), reverse=True)[:k])
...
def selMine(individuals, k, fit_attr="fitness"):
    """Select the *k* best individuals among the input *individuals*. The
    list returned contains references to the input *individuals*.
    ...
    return sorted(individuals, key=attrgetter(fit_attr), reverse=True)[:k]
__all__ = ['selRandom','selMine' , 'selBest', 'selWorst', 'selRoulette',
